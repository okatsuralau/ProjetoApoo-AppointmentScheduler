	...
	<c:when test="${not empty user}">
		<div class="panel panel-default">
			<table class="croogo-table table table-striped table-hover no-mb">
				...
				<tbody>
					<c:forEach var="registro" items="${user}">
						<tr>
							<td>${registro.id}</td>
							<td>${registro.individual.first_name}</td>
							<td>${registro.username}</td>
							<td>${registro.role.title}</td>
							<td>${registro.individual.skin.title}</td>
							<td>${registro.individual.civilstatus.title}</td>
							<td>${registro.individual.educationlevel.title}</td>
